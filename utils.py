...
from collections import defaultdict #使用collections类中的defaultdict()方法来为字典提供默认值
import numpy as np
import pickle
import random
...
CHAR_VOCAB = []
CHAR_VOCAB_BG = []
#第一个参数为default_factory属性提供初始值，默认为None；其余参数包括关键字参数
#当字典中没有的键第一次出现时，default_factory自动为其返回一个空列表，list.append()会将值添加进新列表；
# 再次遇到相同的键时，list.append()将其它值再添加进该列表
w2i = defaultdict(lambda: 0.0)#未知键赋值0
...
WORD_LIMIT = 20213 # remaining 1 for <PAD> (this is inclusive of UNK)
task_name = ""
TARGET_PAD_IDX = -1
INPUT_PAD_IDX = 0
...
def get_batched_input_data(lines, batch_size, rep_list=['swap'], probs=[1]):
    #shuffle(lines)
    total_len = len(lines)
    output = []
    for batch_start in range(0, len(lines) - batch_size, batch_size):
        ...
        y = []
        lens = []
        max_len = max([len(line.split()) for line in lines[batch_start: batch_start + batch_size]])
...
#句子的向量表达
def get_line_representation(line, rep_list=['swap'], probs=[1]):
    rep = []
    modified_words = []
    for word in line.split():
        rep_type = np.random.choice(rep_list, 1, p=probs)[0]
        # np.random.choice(a, size=None, replace=True, p=None)
        #从一维array a 或 int 数字a中，以概率p随机选取大小为size的数据
        ...
        if 'swap' in rep_type:
            ...
            word_rep, new_word = get_drop_word_representation(word, 1)
