...
import numpy as np
from read_files import read_imdb_files, read_yahoo_files, read_agnews_files, read_sst_files,read_toxic_files,read_enron_files
...
def get_tokenizer(dataset):
    ...
    if dataset == 'imdb':
        ...
        texts, _ = read_sst_files("train")
    elif dataset == 'toxic':
        texts, _ = read_toxic_files("train")
    elif dataset == 'enron':
        texts, _ = read_enron_files("train")
...
def word_process(train_texts, train_labels, test_texts, test_labels, dataset):
    maxlen = config.word_max_len[dataset]#imdb400
